...
export default function CadastrarProjetoPage() {
         ...
         apiClient
            ...
            .then((response) => {
               const availableAlunos = response.data.filter(
                  (aluno: Aluno) =>
                     !selectedAlunos.find(
                        (sa) => sa.matriculaAluno === aluno.matriculaAluno
                     )
               );
   ...
   const handleRemoveAluno = (alunoParaRemover: Aluno) => {
      setSelectedAlunos(
         selectedAlunos.filter(
            (aluno) => aluno.matriculaAluno !== alunoParaRemover.matriculaAluno
         )
      );
   };
   ...
   const handleSubmit = async (event: React.FormEvent) => {
      ...
      try {
         ...
         setErro(
            handleApiError(error, "Ocorreu um erro ao cadastrar o projeto.")
         );
      ...
      <div className="min-h-screen bg-gray-50 py-10 px-4 flex justify-center">
         <div className="w-full max-w-3xl bg-white rounded-lg shadow-md p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
               ...
               <div className="flex flex-col">
                  <label
                     htmlFor="bannerProjeto"
                     className="text-sm font-semibold text-gray-700 mb-2"
                  >
                     Banner do Projeto (Opcional)
                  </label>
                  <input
                     type="file"
                     id="bannerProjeto"
